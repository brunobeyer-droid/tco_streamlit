...
def _select_team_group_app(prefix: str) -> Tuple[Optional[str], Optional[str], Optional[str], Optional[str], Optional[str]]:
    ...
    # Use ADD_INFO (replaces legacy SITE)
...
def _search_invoices_enriched(
    ...
    # NOTE: replaced a.SITE with a.ADD_INFO
    sql = f"""
        SELECT
            i.INVOICEID,
            i.FISCAL_YEAR,
    ...
    # Use ADD_INFO instead of legacy SITE
...
with tab_search:
    ...
    with sc5:
        ...
        apps = apps.copy()
        if not apps.empty:
            # Use ADD_INFO for label (legacy SITE removed)
            apps["LABEL"] = apps.apply(lambda r: f"{r['APPLICATIONNAME']}" + (f" — {r['ADD_INFO']}" if r.get("ADD_INFO") else ""), axis=1)
    ...
    if res.empty:
        ...
        pcol1, pcol2, _ = st.columns([0.2, 0.3, 0.5])
        with pcol1:
        ...
        for r in range(rows):
            ...
            row_df = subset.iloc[start_idx:end_idx]
