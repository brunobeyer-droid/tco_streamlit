...
# Switched to helper upserts in snowflake_db (decimal‑safe / idempotent)
...
def ado_features_base_query(where_sql: str = "", params: Optional[tuple] = None) -> pd.DataFrame:
    ...
    if "EFFORT_POINTS" in df.columns:
        # Coerce to numeric to avoid None when decimals exist or source had strings
...
with tab_explore:
    ...
    # NEW: quick repair action for decimal effort points
    b1, _ = st.columns([1, 5])
    with b1:
