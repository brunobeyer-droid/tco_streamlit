...
# Usage:
#   ./scripts/refresh_projected_demand_snapshot.sh [YEAR] [INCLUDE_PRIOR_YEAR] [REFERENCE_YEAR_ONLY]
# Example:
#   ./scripts/refresh_projected_demand_snapshot.sh 2026 1 0
...
echo "Refreshing projected demand snapshot..."
echo "  YEAR=${YEAR}"
...
from db import refresh_tco_projected_demand_snapshot
...
ok = refresh_tco_projected_demand_snapshot(
    year=year,
    include_prior_year=include_prior_year,
    reference_year_only=reference_year_only,
)
print(f"refresh_tco_projected_demand_snapshot={bool(ok)}")
PY
